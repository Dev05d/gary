...
export default function App() {
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      await streamChat(
        { message: text, conversation_id: activeId, role },
        {
          ...
          onError: (message) =>
            setPending((prev) =>
              prev ? { ...prev, streaming: false, error: message } : { id: "err", content: "", model: "", streaming: false, error: message },
            ),
        },
        controller.signal,
      );
    ...
    [activeId, busy, role, loadTurns, refreshConversations],
  );
